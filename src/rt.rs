...
use crate::rt::gc::{mark_gray, heap, OwnedKObject};
...
pub unsafe extern "C" fn krab_gc_allocate(size: usize, align: usize) -> KRef {
    ...
    let index = heap().insert(OwnedKObject(obj)).unwrap();
...
pub unsafe fn gc_deallocate(ptr: *mut u8) {
    ...
    let align = metadata.align.max(align_of::<ObjectHead>());
    let offset = calculate_offset(align);
...
pub const fn calculate_offset(align: usize) -> usize {
